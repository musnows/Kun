...
Kun 桌面版把 `kun-examples.kun-video-editor` 同时作为默认本地扩展和仓库内完整的 Extension API v1.1 参考示例。它只有一份源码：`examples/extensions/kun-video-editor`；产品代码不会导入其实现，也不会通过私有路径注册 contributions。
