...
For directly discoverable extension UI, the canonical v1 entry point is `views.rightSidebar`: each visible View registers its own declared `icon` in Kun's existing right rail and opens an isolated Webview beside the main conversation. Kun does not add a separate aggregate puzzle menu. `views.leftSidebar`, `views.auxiliaryPanel`, `views.editorTab`, and `views.fullPage` remain v1 Schema- and command-compatible, but new extensions should not use them as their default discoverable entry point.
...
- Every visible `views.rightSidebar` View receives its own right-rail button. If an icon is omitted, the Host uses an accessible fallback without executing extension code.
